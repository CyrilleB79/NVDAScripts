...
def diffLine(text1, text2):
	...
	with open(os.path.join(d.name, 'out.txt'), 'w+') as fout:
		with open(os.path.join(d.name, 'err.txt'), 'w+') as ferr:
			...
			err = ferr.read()
			if err:
				RuntimeError(f'Error when executing the following command:\n{cmdParams}\n{err}')
			fout.seek(0)
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	def popupMenu(self, text):
		...
		item = self.menu.Append(
			wx.ID_ANY,
			_("&Compare with reference"),
			_("Compare with reference"),
		)
		self.menu.Bind(wx.EVT_MENU, lambda evt: self.onCompare(text), item)
		gui.mainFrame.prePopup()
		gui.mainFrame.sysTrayIcon.PopupMenu(self.menu)
		gui.mainFrame.postPopup()
		
	def onCompare(self, text):
		try:
			otherText = self.otherText
		except AttributeError:
			msg = _('No text previously saved')
			core.callLater(0.01, lambda: self.actionMessage(msg))
			return
		hasDiff = diffLine(otherText, text)
		if not hasDiff:
			msg = _('No difference')
			core.callLater(0.01, lambda: self.actionMessage(msg))
			return
